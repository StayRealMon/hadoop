...
2. 存储架构分为：**①缓冲区**，s用于暂存ETL数据，正是导入之前用于预处理和缓存 **②数仓**，存放处理后的数据，常见的组织形式有“多维数据库组织形式”和“基于关系数据库形式”**③数据集市**，面向具体的主题和业务部门进行高性能结构查询，具有较粗的粒度和适量的历史数据，响应快 **④元数据**，类似于数据字典，描述源数据库和目标数据库的数据模型、物理结构以及相关匹配模式等重要信息

## 常见的ODS-DW模式 ##
1. **ods**是短期的实时的数据，供产品或者运营人员日常使用，而数据仓库是供战略决策使用的数据；ods是可以更新的数据，数据仓库是基本不更新的反应历史变化的数据
2. **dw**存储是一个面向主题的，反映历史变化数据，用于支撑管理决策

### 业务数据 － ODS － 数据仓库 ###
![](https://images2015.cnblogs.com/blog/153316/201511/153316-20151108174624102-604335425.png)
...
### ods-dw并行 ###
![](https://images2015.cnblogs.com/blog/153316/201511/153316-20151108181704727-1347319321.png)
...
**星型(1×事实表，n×维度表)雪花型(1×事实表，n1×n2维度表)星座型(m×事实表，n×维度表)**
...
## Hive表规范 ##
**数据库：stg**(临时缓冲)/**ods**(源数据，pg表抽取来的)/**dw**(核心数仓存放主题域粒度表)/**dm**(数仓集市)/**st**(应用展现，展示固定的报表，单个应用的底层数据)/**dim**(维度表)
1. ods表名和pg表一样；dw、dm、st层表名采用数据库名_主题域_自定义表名组成；tmp_表名_自定义名称_yyyymmdd，默认yyyymmdd与日期一致
2. ods层统一使用TEXTFILE;dw 、dm、st层首选使用OrcFile，也可以部分使用TEXTFILE;dim层使用TEXTFILE或orcfile
3. hive 表字段分隔符使用  \001;hive 表行分隔符使用 \n
4. **全量表分区** ：分区存放截止到T-1天的所有历史数据，全量表历史分区数据不允许删除，由hadoop管理员统一管理
5. **增量表分区**：分区存放的是以业务时间戳、创建时间戳等于分区日期的数据，每一条数据只能存在于一个分区中

## ods规范 ##
```bash
	source ~/hive_config.sh                                   ##公共变量文件
	
	db_name="ods"                                          ##数据库名
	table_name="t_bike_info"。      ##表名
	
	hive\<\< EOF
	
	use  ${db_name};
	DROP TABLE IF EXISTS ${table_name};          
	CREATE EXTERNAL TABLE ${table_name}              ##外部表
	(
	guid                  string                comment 'guid',           ##   字段类型对齐，comment对齐      
	bike_no            string                comment 'bike编号',    ##   必须要有备注
	city_guid          string                comment '城市cuid',    
	city_name        string                comment '城市名称',
	produce_time  timestamp         comment '生产时间'
	)
	COMMENT '车辆信息表'                                              ##表注释必须要
	PARTITIONED BY(pt STRING)                                   ##pt表示日期分区
	ROW FORMAT DELIMITED FIELDS TERMINATED BY '\001' LINES TERMINATED BY '\n'     ##统一分隔符
	STORED AS TEXTFILE                                              #ods层统一用TEXTFILE
	
	LOCATION 'hdfs://${hadoop_cluster}/user/hive/dataware/${db_name}/${table_name}'  ; 
	
	ALTER TABLE ${table_name} SET SERDEPROPERTIES('serialization.null.format'='');
	EOF
```

1. **增量抽取**：对于数据量大的表，采用增量抽取数据与上一天的全量数据合并，落地到全量表分区；对于增量表分区，之际抽取增量数据到对应的增量表日期分区中，如t_ride_info
2. **全量抽取**：数据量小的表，每天全量抽取所有数据到当天分区中


## 数据脱敏 ##
1. ods以上开放数据层全部脱敏，数据应用不可查 ; 同时保存脱敏前的数据，如市场营销或特殊情况需要完整敏感信息,恢复时还是可以从hive恢复，不需要直接从数据库再同步
2. 脱敏方法和技术：在XX层保留字段主键+脱敏前源字段信息（基于部分表比较大不适合保存全量）；在ods层实现最终脱敏后的字段信息。简单的规则数据如手机号身份证号可以采用hive函数substring、concat

## 体系架构 ##
1. **外部系统**：外部数据/RDBMS/打点埋点/http&ftp&txt
2. sqoop抽取同步到源数据存储 **ods**层
3. ETL清洗存放到 **dwd**，轻微汇总到 **dws**
4. dwd汇总装载到多维数仓 **dm**
5. dm和dws汇总到 **st**数据展现层，用于宽表用户画像和报表
6. dm提供外部接口，对外提供数据服务如数据导出/报表工具/即时查询

## 模型层次 ##
源操作数据层 （ stg、ods）、核心维度模型层（dw）、应用数据层 （dm、st)

### 概念模型 ###
实体：单车/助力车/订单/客户/客服/财务/营销
### 逻辑模型 ###
1. 区分主题域，两大主要业务单车助力车分为十个主题域：单车、助力车、订单、客户、客服、市场、财务、(直租车、打车、顺风车)
2. 单车域包含单车生产、投放、运营，不包含单车相关的订单和用户

### 物理模型 ###
生成(基于hadoop&hive的事实表和维度表)建表脚本，新建表